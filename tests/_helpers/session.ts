...
/**
 * Delete a user via admin API. Used by `afterEach` to clean up ephemeral
 * users created during a single test (e.g. user B in Risk #1 E2E spec).
 * Throws on failure so a leak is surfaced rather than silently accumulating
 * orphan rows in `auth.users`.
 */
export async function deleteUser(userId: string): Promise<void> {
  const supabaseAdmin = adminClient();
  const { error } = await supabaseAdmin.auth.admin.deleteUser(userId);
  if (error) throw new Error(`admin deleteUser(${userId}) failed: ${error.message}`);
}
