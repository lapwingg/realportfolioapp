...
**Fund-conversion cutoff**: when the Allianz CSV contains a `Zamiana` (fund-conversion) row, the importer at `src/lib/allianz/parse.ts` captures it as a synthetic `transactions` row with `source='carryover'`, `units = Liczba jednostek (fundusz docelowy)`, `gross_amount = 0`. The dashboard's valuation helper at `src/lib/valuation/compute.ts` derives the cutoff as `MAX(transaction_date) WHERE source='carryover'` and includes only rows dated on or after that cutoff in the SUM — so pre-conversion OLD-fund units are excluded from the valuation while staying in the table for S-03's tax calculations. A subtle Polish footnote under the "Pobrano" line ("Wycena uwzględnia jednostki od konwersji z dnia DD.MM.YYYY") names the active cutoff. Users with no Zamiana in their history see unchanged behavior. Run `npm run verify-valuation` to exercise the helper.
