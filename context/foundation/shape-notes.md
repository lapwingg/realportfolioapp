...
created: 2026-06-23
updated: 2026-06-23
# session finalised 2026-06-23
checkpoint:
  current_phase: 8
  phases_completed: [1, 2, 3, 4, 5, 6, 7]
  gray_areas_resolved:
    - topic: "pain category"
      decision: "data trapped in wrong form + missing calculation capability"
    - topic: "persona scope"
      decision: "individual PPK holders broadly"
    - topic: "insight"
      decision: "PPK providers have no incentive to surface the after-tax number — gross AUM looks better"
    - topic: "auth model"
      decision: "login (email + password or OAuth); flat user model; each user sees only their own data"
    - topic: "mvp scope"
      decision: "full flow committed: import, categorise, scrape price, all three withdrawal scenarios; extras deferred to v2"
    - topic: "product type"
      decision: "web-app"
    - topic: "target scale"
      decision: "small (just me or a handful)"
    - topic: "deadline"
      decision: "2026-07-05 hard deadline; after-hours work; tension with 4-week estimate flagged as open question"
  frs_drafted: 10
...
The insight: PPK providers have no incentive to surface the after-tax reality. Displaying gross AUM makes returns look larger and engagement higher. The raw data exists (contribution history, fund unit prices, tax rules), but no platform assembles it into a per-scenario, after-tax answer for the individual holder.
...
Login required (email + password or OAuth). Flat user model — every authenticated user sees only their own data. No admin role, no guest access, no team or sharing features in MVP.
...
The user encounters the output as a dashboard shown after uploading their file and clicking "fetch price" — all three scenario amounts appear simultaneously, not behind separate navigation.
...
- Tax calculation correctness is non-negotiable: the after-tax amount displayed must reflect actual Polish tax rules for the scenario; silent estimation or rounding errors are not acceptable.
- The full price fetch + calculation cycle completes within a few seconds of the user triggering it (target: visible result in under 5 seconds under normal network conditions).
- The product is usable on all major desktop browsers (Chrome, Firefox, Safari, Edge — latest two major versions each).

## Timeline acknowledgment
mvp_weeks: 4 (after-hours). Full flow committed with all three withdrawal scenarios; user accepted the timeline with awareness of the sustained-effort cost.
hard_deadline: 2026-07-05. Note: 12 days from session date (2026-06-23) — significantly shorter than the 4-week estimate. Tension recorded as open question.
